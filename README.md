...
| | | |
|---|---|---|
| **1** | [Check the plan against policies in this repo](../../pull/11) | Three rules committed under `.tirith/policies/`, evaluated on the runner. **No StackGuardian account, no API key, no network call** — you can reproduce this one today with nothing but the files in the diff. |
| **2** | [Take the policies from the organization instead](../../pull/12) | Two lines in, three files out. The rules move to the org, and two engines appear that a runner cannot run: cost, which needs the plan priced, and rego, which needs an OPA engine. |
| **3** | [Add the analytics bucket](../../pull/13) | An ordinary-looking change that ships with its `Owner` tag left blank. The check goes red and `Apply` is skipped. Nobody had to remember to look. |
| **4** | [Give the analytics bucket an owner](../../pull/14) | One line. The gate clears. Governance is a step in the workflow, not a wall across it. |
| **5** | [Publish the terraform state after apply](../../pull/15) | A second call to the same action, this time carrying the state, so the platform holds what is actually deployed and not just what was proposed. |
...
| Action | `StackGuardian/tirith-iac-governance-action@v2.1.0` (py-tirith 1.2.1) |
...
Every comment leads with the **planned changes** as a `diff` block — one row per changing resource,
the attributes that move underneath it, and terraform's summary line — so the thing a rule objected to
is visible in the change itself and not only in the verdict. Chapter 3 is where that pays off: the
empty `Owner` sits in the plan directly above the rule that refused it. It is rendered from the masked
plan on the runner, so a value terraform marked sensitive never reaches the comment, and unchanged
resources are counted rather than listed.
